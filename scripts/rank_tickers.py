...
class TickerRanker:
    ...
    async def analyze_ticker(self, ticker: str) -> Dict:
        ...
        # Extract headlines
        headlines = [n.get("headline", "") for n in news[:10]] # Limit to 10 for context
        news_text = "\n".join(headlines)
    ...
    async def rank_all_tickers(self):
        ...
        logger.info(f"--- Morning Ticker Ranking ({datetime.now().strftime('%Y-%m-%d')}) ---")
        ...
        ranked = sorted(results, key=lambda x: (x['confidence'], x['volume']), reverse=True)
        ...
        for i, res in enumerate(top_3, 1):
            sent_icon = "📈" if res['sentiment'] > 0 else "📉" if res['sentiment'] < 0 else "😐"
            print(f"| {i} | **{res['ticker']}** | {res['confidence']}% | {sent_icon} {res['sentiment']:.2f} | {res['volume']} | {res['reason']} |")
        print("\n")
