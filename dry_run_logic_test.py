...
with patch("bot.portfolio_reconciler.PortfolioReconciler"), patch(
    "bot.ticker_ranker.TickerRanker"
), patch("bot.feedback_agent.FeedbackAgent"), patch(
    "bot.fundamental_agent.FundamentalAgent"
), patch(
    ...
    pm_instance.get_held_tickers.return_value = {
        "TSLA": 10
    }  # Holding 10 shares of TSLA
    ...
    async def mocked_run_audit():
        ...
        with patch(
            "bot.main.get_macro_context", new_callable=MagicMock
        ) as mock_macro, patch("bot.main.fetch_historical_data") as mock_hist, patch(
            "bot.main.fetch_ticker_intelligence"
        ) as mock_intel, patch(
            "bot.main.fundamental_agent"
        ) as mock_fa, patch(
            "bot.main.reconciler"
        ) as mock_rec:
            ...
            for res in results:
                print(
                    f"[{res.get('ticker')}] SIGNAL: {res.get('signal')} | REASON: {res.get('reason')}"
                )
