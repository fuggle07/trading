...
async def verify_macro_feeds():
    print("🚀 Verifying Macro Intelligence Feeds (with Finnhub Fallback & GCP Secrets)...")
    ...
    if not (fmp_key and finnhub_key and av_key):
        ...
        try:
            ...
            if not fmp_key: fmp_key = get_secret("FMP_KEY")
            if not finnhub_key: finnhub_key = get_secret("FINNHUB_KEY")
            if not av_key: av_key = get_secret("ALPHA_VANTAGE_KEY")
    ...
    if calendar:
        for ev in calendar:
            print(f"📅 {ev.get('date')} | {ev.get('event')} | Impact: {ev.get('impact')}")
    else:
    ...
    if rates:
        print(f"📈 10Y Yield: {rates.get('10Y')}% | 2Y Yield: {rates.get('2Y')}% (Date: {rates.get('date')})")
    else:
    ...
    if indices:
        print(f"📉 VIX: {indices.get('vix')} | SPY: {indices.get('spy_perf')}% | QQQ: {indices.get('qqq_perf')}%")
    else:
